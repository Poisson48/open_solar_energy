...
# AppImage local
VERSION_NAME=2.0.4 QT_ROOT="$HOME/Qt/6.8.2/gcc_64" bash scripts/build-appimage.sh
```
...
VERSION_NAME=2.0.4 VERSION_CODE=204 bash scripts/build-android.sh
...
```bash
git tag -a v2.0.4 -m "Notes affichées dans l’app avant MAJ" && git push origin v2.0.4
```
