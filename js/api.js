...
const AppAPI = (() => {
  ...
  function setSizing(params = {}) {
    ...
    if (params.tech           !== undefined) setField('sz-tech',            params.tech);
  ...
  function setOffgrid(params = {}) {
    ...
    if (params.bosCost        !== undefined) setField('og2-bos-cost',          params.bosCost);
...
AppAPI.setSizing({ monthlyKwh:[...12], tariff, priceBase, priceHp, priceHc, subscription, costKwp, strategy, targetCoverage, feedin, surface, tech })
AppAPI.setOffgrid({ dailyDefault (Wh/j !), dailyByMonth:[...12] (Wh/j), battTech, targetCoverage, pvCostKwp, bosCost, surface })
  Stratégies sizing : 'bill_coverage_pct' | 'autoconso_max' | 'roi_optimal' (défaut: roi_optimal)
...
Résultats sizing  : Ppeak, nPanels, surfaceNeeded, annualProd, coverageRate (%), savedOnBill, feedinRevenue, totalAnnualGain, newAnnualBill, paybackYears, npv25, lcoe, incentive, systemCost
Résultats offgrid : Ppeak, C_batt_gross, C_usable, coverageRate (%), systemCost, deficit_days, autonomyDays, total_conso, total_deficit

Exemple :
  const r = AppAPI
    .setInstall({ tilt: 30, azimuth: 0, surface: 25 })
    .setSizing({ monthlyKwh: [350,300,280,250,240,230,240,250,270,310,340,360], strategy: 'bill_coverage_pct', targetCoverage: 80 })
    .calc('sizing');
  console.table(r);
`);
  }
