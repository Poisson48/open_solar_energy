...
const Charts = (() => {
  ...
  function renderHourlyProfile(canvasId, sim, monthName) {
    ...
    new Chart(document.getElementById(canvasId), {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Production PV (Wh)',    data: sim.map(r => Math.round(r.pv * 1000)),
            backgroundColor: 'rgba(245,166,35,0.75)', order: 2 },
  ...
  // ── Graphique état de charge batterie ─────────────────────────
  function renderHourlySoc(canvasId, sim, usableKwh) {
    const labels = sim.map(r => `${String(r.hour).padStart(2,'0')}h`);
    new Chart(document.getElementById(canvasId), {
      type: 'line',
      data: {
        labels,
        datasets: [
          { label: 'SoC batterie (kWh)', data: sim.map(r => r.soc),
          ...
          { label: `SoC max (${usableKwh.toFixed(1)} kWh)`,
            ...
            borderColor: '#4caf50', borderDash: [6,3], borderWidth: 1.5,
  ...
  return {
    ...
    renderHourlyProfile, renderHourlySoc,
    destroy
  };
})();
